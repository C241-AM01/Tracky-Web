...
function eventListenerDropdown1(event, subitem, item, place) {
    ...
    var popper = Popper.createPopper(
        subitem,
        item.querySelector("#productListTable .dropdown-menu"),
        {
            placement: place,
        }
    );
    ...
    if (subitem.classList.contains("show") != true) {
        item.querySelector("#productListTable .dropdown-menu").classList.remove(
            "block"
        );
        item.querySelector("#productListTable .dropdown-menu").classList.add(
            "hidden"
        );
    } else {
        dismissDropdownMenu();
        ...
        if (
            item
                .querySelector("#productListTable .dropdown-menu")
...
function dismissDropdownMenu() {
    Array.from(
        document.querySelectorAll("#productListTable .dropdown-menu")
...
function dropdownEvent1(elem, place, isRemove = null) {
    if (isRemove) {
        ...
        Array.from(elem).forEach(function (item) {
            item.querySelectorAll(".dropdown-toggle").forEach(function (
                subitem
            ) {
...
function updateDropdown() {
    setTimeout(() => {
        ...
        var allElements = document.querySelectorAll(
            "#productListTable .dropdown"
        );

        // Filter elements that do not have a click event listener
        var elementsWithoutClickListener = Array.from(allElements).filter(
            function (element) {
...
var productListTable = new List("productListTable", options).on(
    "updated",
    function (list) {
        ...
        const totalRecordsElement =
            productListTable.listContainer.querySelector(".total-records");
        const showingElement =
            productListTable.listContainer.querySelector(".showing");

        if (totalRecordsElement)
            totalRecordsElement.innerHTML = productListTable.items.length;
        if (showingElement)
            showingElement.innerHTML = productListTable.visibleItems.length;
        document
            .querySelector(".pagination-next")
...
xhttp.onload = function () {
    ...
    Array.from(json_records).forEach(function (element) {
        productListTable.add({
            ...
            product_name:
                `<a href="asset-detail-admin" class="flex items-center gap-2">
                            <img src="` +
                element.img +
            ...
            action: `<div class="relative dropdown">
                    ...
                    <ul class="absolute z-50 hidden py-2 mt-1 ltr:text-left rtl:text-right list-none bg-white rounded-md shadow-md dropdown-menu min-w-[10rem] dark:bg-zink-600" aria-labelledby="productAction1">
                        <li>
                            <a class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-500 dark:hover:text-zink-200 dark:focus:bg-zink-500 dark:focus:text-zink-200" href="asset-detail-admin"><i data-lucide="eye" class="inline-block w-3 h-3 mr-1"></i> <span class="align-middle">Overview</span></a>
                        </li>
                        <li>
                            <a class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-500 dark:hover:text-zink-200 dark:focus:bg-zink-500 dark:focus:text-zink-200" href="edit-asset-admin"><i data-lucide="file-edit" class="inline-block w-3 h-3 mr-1"></i> <span class="align-middle">Edit</span></a>
                        </li>
                        <li>
    ...
    var pageLinks = document.querySelectorAll(
        "#productListTable .listjs-pagination .page"
    );
